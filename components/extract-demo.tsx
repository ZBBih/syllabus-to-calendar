...
function dateLabel(date: string, endDate?: string): string {
  ...
  return sameMonth
    ? `${dayLabel(date)}–${Number(endDate.slice(8))}`
    : `${dayLabel(date)} – ${dayLabel(endDate)}`;
}
...
function build() {
  ...
  const certainty = new Map(
    events.map((e) => [`${e.date}|${e.title.toLowerCase()}`, e.confidence]),
  );
...
export function ExtractDemo({ onSample }: { onSample?: () => void }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      () => setShown((s) => (s >= text.length ? s : s + 1)),
      STEP,
    );
  ...
  return (
    <section aria-labelledby="demo-heading">
      ...
      <p className="lede mx-auto mt-2 text-center">
        This is the real reader, running in your browser, on a real syllabus.
      ...
      <div className="card mt-8 grid gap-px overflow-hidden bg-line md:grid-cols-2">
            ...
            <ol aria-label="The syllabus">
              {lines.map((line, i) => (
                <li
                  key={`${i}-${line}`}
                  className={
                    i === shown - 1 && !done ? "text-fg" : "text-muted"
                  }
                >
                  {line}
                </li>
        ...
        <div className="bg-elev p-4 sm:p-5">
          ...
          <div
            ref={rowScroller}
            className="mt-3 h-[19.25rem] overflow-hidden sm:h-[24.5rem]"
          >
            <ul aria-label="What it found">
              {visible.map((r, i) => (
                <li
                  key={r.key}
                  className="pop-in flex h-7 items-center gap-3 text-[0.9375rem]"
                >
                  <span
                    className={`dot ${DOT[i % DOT.length]}`}
                    aria-hidden="true"
                  />
            ...
            {visible.length === 0 && (
              <p className="text-sm text-muted">
                Reading {term} out of the dates on the page…
              </p>
            )}
          </div>
        </div>
      ...
      {done && (
        ...
        >
          {meetingLine && (
            <div className="card p-4">
              <p className="flex items-center gap-2 text-sm font-semibold">
                <Check size={15} className="text-accent" /> The weekly class, as
                a repeating event
              </p>
          ...
          {weights.length > 0 && (
            <div className="card p-4">
              <p className="flex items-center gap-2 text-sm font-semibold">
                <Check size={15} className="text-accent" /> The grading table,
                as a grade calculator
              </p>
