...
export function Landing({ dispatch }: { dispatch: Dispatch<Action> }) {
  ...
  return (
    <div className="step-enter">
      {/* On a wide screen the green runs edge to edge; the words and the picture stay on the
          page column inside it, so they line up with everything underneath. */}
      <section className="block block-hero bleed">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-6 py-10 sm:px-10 sm:py-14 lg:grid-cols-[1.35fr_1fr] lg:gap-14 lg:px-6 lg:py-24">
          <div>
            <h1 className="h-display">
              Your whole semester, on your calendar,{" "}
              <em className="text-hero-amber">in one minute.</em>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <button
                type="button"
                onClick={start}
                className="btn btn-on-hero btn-hero"
              >
              ...
              <button
                type="button"
                onClick={sample}
                className="btn btn-ghost-on-hero btn-hero"
              >
                See it on a sample
              </button>
            </div>
            <p className="mt-4 text-sm text-hero-quiet">
          ...
          <HeroArt
            onHero
            className="order-first mx-auto w-full max-w-md lg:order-none lg:max-w-none"
          />
        </div>
      </section>
