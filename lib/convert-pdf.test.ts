  ...
  getDocument: () => ({
    promise: Promise.resolve({
      ...
      getPage: async (p: number) => {
        ...
        inFlight -= 1
        return {
          getTextContent: async () => ({ items: [{ str: `page ${p}`, transform: [0, 0, 0, 0, 0, 100] }] }),
        }
      },
    }),
  }),
...
describe('reading a PDF several pages at a time', () => {
  it('keeps the pages in document order however they come back', async () => {
    ...
    const text = await fileToText(new File([new Uint8Array(4)], 'syllabus.pdf', { type: 'application/pdf' }))
    const lines = text.split('\n').filter(Boolean)

    expect(lines).toEqual(Array.from({ length: PAGES }, (_, i) => `page ${i + 1}`))
  })

  it('reads more than one page at a time, and never more than the batch', async () => {
    const { fileToText } = await import('./convert')
    peak = 0
    await fileToText(new File([new Uint8Array(4)], 'syllabus.pdf', { type: 'application/pdf' }))
